...
import {DiceType, HitLocation} from "./gameEntitiesTypes.js";
...
export const calculateHpModifier = (characteristics) => {
  const {SIZ, POW, CON} = characteristics;
  const sizeModifier = SIZ <= 4 ? -2 : -2 + Math.ceil((SIZ - 4) / 4);
  const powerModifier = POW <= 4 ? -1 : POW <= 16 ? 0 : Math.ceil((POW - 16) / 4);
  return CON + sizeModifier + powerModifier;
};
export const calculateMaxLocationsHp = (totalHp) => ({
...
export const calculateHealingRate = (con) => Math.max(1, Math.ceil(con / 6));
export const calculateDamageBonus = (characteristics) => {
  const {SIZ, CON} = characteristics;
  const sum = SIZ + CON;
  if (sum <= 12)
    return {type: DiceType.D4, isNegative: true};
  if (sum <= 24)
    return null;
  if (sum <= 32)
    return {type: DiceType.D4};
  if (sum <= 40)
    return {type: DiceType.D6};
  return {type: DiceType.D6, quantity: 1 + Math.ceil((sum - 40) / 16)};
};
export const calculateSpiritCombatDamage = (characteristics) => {
  const {POW, CHA} = characteristics;
  const sum = POW + CHA;
  if (sum <= 12)
    return {type: DiceType.D3};
  if (sum <= 24)
    return {type: DiceType.D6};
  if (sum <= 32)
    return {type: DiceType.D6, fixedPart: 1};
  if (sum <= 40)
    return {type: DiceType.D6, fixedPart: 3};
  if (sum <= 56)
    return {type: DiceType.D6, quantity: 2, fixedPart: 3};
  const timesOf16 = Math.ceil((sum - 56) / 16);
  return {type: DiceType.D6, quantity: 2 + timesOf16, fixedPart: 3 + timesOf16};
};
export const calculateEncumbrancePoints = (characteristics) => {
  const {STR, CON} = characteristics;
  return STR < CON ? STR : Math.round((STR + CON) / 2);
};
export const calculateDexStrikeRank = (dex) => {
  if (dex <= 5)
    return 5;
  if (dex <= 8)
    return 4;
...
export const calculateSizStrikeRank = (siz) => {
  if (siz <= 6)
    return 3;
  if (siz <= 14)
    return 2;
